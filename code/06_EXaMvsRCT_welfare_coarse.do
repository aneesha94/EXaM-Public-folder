...
*	Draw Histograms
	preserve
		* WTP
		...
		save "$EXaM_temp/06_WTP_EXaM_coarse_$simulation.dta", replace
		use "$EXaM_temp/06_WTP_EXaM_coarse_$simulation.dta", clear
		...
		graph export "$EXaM_output/06_EXaMvsRCT_welfare_WTP_coarse_medi_$simulation.png", replace
		graph save "$EXaM_output/06_EXaMvsRCT_welfare_WTP_coarse_medi_$simulation", replace
		...
		graph export "$EXaM_output/06_RCT_welfare_WTP_coarse_medi_$simulation.png", replace
		graph save "$EXaM_output/06_RCT_welfare_WTP_coarse_medi_$simulation", replace
		...
		twoway (histogram WTP_RCT_ if WTP_RCT_ >= -20 & WTP_RCT_ <= 60, frac scheme(s1manual) width(0.75) graphregion(lcolor(none)) plotregion(style(none) margin(small)) xtitle("{bf:WTP Measured by Time Cost of Water Collection} (Unit: Workdays)", size(medium) color(gs0)) ///
			...
			fcolor(none) lwidth(thin) lcolor(gs0)), legend(label(1 "RCT") label(2 "EXAM") position(6) symysize(*.5) forces) 
		graph export "$EXaM_output/06_EXaMvsRCT_welfare_WTP_coarse_mean_$simulation.png", replace
		graph save "$EXaM_output/06_EXaMvsRCT_welfare_WTP_coarse_mean_$simulation", replace
		...
		twoway (histogram WTP_RCT_ if WTP_RCT_ >= -20 & WTP_RCT_ <= 60, frac scheme(s1manual) width(0.75) graphregion(lcolor(none)) plotregion(style(none) margin(small)) xtitle("{bf:WTP Measured by Time Cost of Water Collection} (Unit: Workdays)", size(medium) color(gs0)) ///
			...
			(histogram WTP_EXaM_ if WTP_EXaM_ >= -20 & WTP_EXaM_ <= 60, frac width(0.75) ///
			fcolor(none) lwidth(none)), legend(label(1 "RCT") label(2 "          ") position(6) symysize(*.5) forces) 
		graph export "$EXaM_output/06_RCT_welfare_WTP_coarse_mean_$simulation.png", replace
		graph save "$EXaM_output/06_RCT_welfare_WTP_coarse_mean_$simulation", replace
	...
	preserve
		* HTE
		...
		twoway (histogram HTE_RCT_ if HTE_RCT_ >= 0 & HTE_RCT_ <= 8, frac scheme(s1manual) width(0.085) graphregion(lcolor(none)) plotregion(style(none) margin(small)) xtitle("{bf:TE Measured by % Reduction in Child Diarrhea}", size(medium) color(gs0)) ///
			...
			fcolor(none) lwidth(thin) lcolor(gs0)), legend(label(1 "RCT") label(2 "EXAM") position(6) symysize(*.5) forces) 
		graph export "$EXaM_output/06_EXaMvsRCT_welfare_HTE_coarse_medi_$simulation.png", replace
		graph save "$EXaM_output/06_EXaMvsRCT_welfare_HTE_coarse_medi_$simulation", replace
		...
		twoway (histogram HTE_RCT_ if HTE_RCT_ >= 0 & HTE_RCT_ <= 8, frac scheme(s1manual) width(0.085) graphregion(lcolor(none)) plotregion(style(none) margin(small)) xtitle("{bf:TE Measured by % Reduction in Child Diarrhea}", size(medium) color(gs0)) ///
			...
			(histogram HTE_EXaM_ if HTE_EXaM_ >= 0 & HTE_EXaM_ <= 8, frac width(0.085) ///
			fcolor(none) lwidth(none)), legend(label(1 "RCT") label(2 "          ") position(6) symysize(*.5) forces) 
		graph export "$EXaM_output/06_RCT_welfare_HTE_coarse_medi_$simulation.png", replace
		graph save "$EXaM_output/06_RCT_welfare_HTE_coarse_medi_$simulation.png", replace
		...
		twoway (histogram HTE_RCT_ if HTE_RCT_ >= 0 & HTE_RCT_ <= 8, frac scheme(s1manual) width(0.085) graphregion(lcolor(none)) plotregion(style(none) margin(small)) xtitle("{bf:TE Measured by % Reduction in Child Diarrhea}", size(medium) color(gs0)) ///
			...
			fcolor(none) lwidth(thin) lcolor(gs0)), legend(label(1 "RCT") label(2 "EXAM") position(6) symysize(*.5) forces) 
		graph export "$EXaM_output/06_EXaMvsRCT_welfare_HTE_coarse_mean_$simulation.png", replace
		graph save "$EXaM_output/06_EXaMvsRCT_welfare_HTE_coarse_mean_$simulation.png", replace
		...
		twoway (histogram HTE_RCT_ if HTE_RCT_ >= 0 & HTE_RCT_ <= 8, frac scheme(s1manual) width(0.085) graphregion(lcolor(none)) plotregion(style(none) margin(small)) xtitle("{bf:TE Measured by % Reduction in Child Diarrhea}", size(medium) color(gs0)) ///
			...
			(histogram HTE_EXaM_ if HTE_EXaM_ >= 0 & HTE_EXaM_ <= 8, frac width(0.085) ///
			fcolor(none) lwidth(none)), legend(label(1 "RCT") label(2 "          ") position(6) symysize(*.5) forces) 
		graph export "$EXaM_output/06_RCT_welfare_HTE_coarse_mean_$simulation.png", replace
		graph save "$EXaM_output/06_RCT_welfare_HTE_coarse_mean_$simulation.png", replace
...
*	Draw CDFs
	preserve
		...
		*******Kolmogorov–Smirnov equality-of-distributions test
		...
		graph export "$EXaM_output/06_EXaMvsRCT_welfare_WTP_coarse_CDF_medi_$simulation.png", replace
		graph save "$EXaM_output/06_EXaMvsRCT_welfare_WTP_coarse_CDF_medi_$simulation", replace
		...
		graph export "$EXaM_output/06_EXaMvsRCT_welfare_WTP_coarse_CDF_mean_$simulation.png", replace
		graph save "$EXaM_output/06_EXaMvsRCT_welfare_WTP_coarse_CDF_mean_$simulation", replace
		...
		* WTP RCT
		...
		graph export "$EXaM_output/06_RCT_welfare_WTP_coarse_CDF_medi_$simulation.png", replace
		graph save "$EXaM_output/06_RCT_welfare_WTP_coarse_CDF_medi_$simulation", replace
		...
		graph export "$EXaM_output/06_RCT_welfare_WTP_coarse_CDF_mean_$simulation.png", replace
		graph save "$EXaM_output/06_RCT_welfare_WTP_coarse_CDF_mean_$simulation", replace
	restore
	...
	preserve
		* HTE
		...
		graph export "$EXaM_output/06_EXaMvsRCT_welfare_HTE_coarse_CDF_medi_$simulation.png", replace
		graph save "$EXaM_output/06_EXaMvsRCT_welfare_HTE_coarse_CDF_medi_$simulation", replace
		...
		graph export "$EXaM_output/06_EXaMvsRCT_welfare_HTE_coarse_CDF_mean_$simulation.png", replace
		graph save "$EXaM_output/06_EXaMvsRCT_welfare_HTE_coarse_CDF_mean_$simulation", replace
		...
		* HTE RCT		
		...
		graph export "$EXaM_output/06_RCT_welfare_HTE_coarse_CDF_medi_$simulation.png", replace
		graph save "$EXaM_output/06_RCT_welfare_HTE_coarse_CDF_medi_$simulation", replace
		...
		graph export "$EXaM_output/06_RCT_welfare_HTE_coarse_CDF_mean_$simulation.png", replace
		graph save "$EXaM_output/06_RCT_welfare_HTE_coarse_CDF_mean_$simulation", replace
...
*	Draw Kernel Densities
	preserve
		* WTP
		...
		graph export "$EXaM_output/06_EXaMvsRCT_welfare_WTP_coarse_density_medi_$simulation.png", replace
		graph save "$EXaM_output/06_EXaMvsRCT_welfare_WTP_coarse_density_medi_$simulation", replace
		...
		graph export "$EXaM_output/06_EXaMvsRCT_welfare_WTP_coarse_density_mean_$simulation.png", replace
		graph save "$EXaM_output/06_EXaMvsRCT_welfare_WTP_coarse_density_mean_$simulation", replace
	restore
	...
	preserve
		* HTE
		...
		graph export "$EXaM_output/06_EXaMvsRCT_welfare_HTE_density_coarse_medi_$simulation.png", replace
		graph save "$EXaM_output/06_EXaMvsRCT_welfare_HTE_density_coarse_medi_$simulation.png", replace
		...
		graph export "$EXaM_output/06_EXaMvsRCT_welfare_HTE_density_coarse_mean_$simulation.png", replace
		graph save "$EXaM_output/06_EXaMvsRCT_welfare_HTE_density_coarse_mean_$simulation", replace
	restore
