...
if NV>0 & NF>0
  % param=[F;B(IDV(:,2)~=5,1);W;A(IDV(:,2)==9,1)];
  param=[F;B(IDV(:,2)~=5 &IDV(:,2)~=9,1);W(IDV(:,2)~=9,1);A];
elseif NV>0 & NF==0
   %param=[B(IDV(:,2)~=5,1);W;A(IDV(:,2)==9,1)];
   param=[B(IDV(:,2)~=5 &IDV(:,2)~=9,1);W(IDV(:,2)~=9,1);A];
elseif NV==0 & NF>0;
   param=F;
else
...
if NV>0
  if sum(IDV(:,2) == 5) >0;
     ...
     bhat=[paramhat(NF+1:NF+sum(IDV(:,2) ~= 9),1);paramhat(end)];
     bsd=[stderr(NF+1:NF+sum(IDV(:,2) ~= 9),1);stderr(end)];
     what=paramhat(NF+sum(IDV(:,2) ~= 9)+1:end,1);
     wsd=stderr(NF+sum(IDV(:,2) ~= 9)+1:end,1);
       else;
