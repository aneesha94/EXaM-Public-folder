...
if sum(p==0)>0;  %Check to see if any probabilities are zero
    disp([num2str(sum(p==0)) ' observations have probability = 0 in this iteration.']);
    p=p.*(p > 0) + (10^(-20)).*(p == 0);   %set zeros to very low values
end;
